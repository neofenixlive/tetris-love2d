...
function love.load()
    ...
    pieces = {
        ...
        J={0,0,0,0,0,0,2,0,0,0,2,0,0,2,2,0},
        S={0,0,0,0,0,3,3,0,3,3,0,0,0,0,0,0},
        Z={0,0,0,0,4,4,0,0,0,4,4,0,0,0,0,0},
        T={0,0,0,0,0,5,0,0,5,5,5,0,0,0,0,0},
...
function love.draw()
    ...
    --room pieces
    for pos, code in ipairs(room) do
        if code ~= 0 then
            local x, y = 0, 0
            for i = 1, pos, 1 do
                if x >= room_width then
            ...
            love.graphics.rectangle("fill", x*40-40, y*40, 40, 40)
            love.graphics.setColor(0, 0, 0)
            love.graphics.rectangle("line", x*40-40, y*40, 40, 40)
        end
    end
    
    --falling piece
    for pos, code in ipairs(falling) do
        if code ~= 0 then
            local x, y = 0, 0
            for i = 1, pos, 1 do
                if x >= 4 then
            ...
            love.graphics.setColor(pieces_colors[code][1], pieces_colors[code][2], pieces_colors[code][3])
            love.graphics.rectangle("fill", (x+falling_x)*40-40, (y+falling_y)*40, 40, 40)
            love.graphics.setColor(0, 0, 0)
            love.graphics.rectangle("line", (x+falling_x)*40-40, (y+falling_y)*40, 40, 40)
        end
    end
    
    --next piece
    for pos, code in ipairs(game.next_piece) do
        local x, y = 0, 0
        for i = 1, pos, 1 do
            if x >= 4 then
                x = 1
        ...
        love.graphics.rectangle("fill", (x+9)*40, (y+1.5)*40, 40, 40)
    end
